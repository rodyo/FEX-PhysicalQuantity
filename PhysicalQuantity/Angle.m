classdef Angle < PhysicalQuantityInterface
    ...
    methods (Static)
        ...
        function new_obj = asin(A,varargin)
            try new_obj = Angle.inverseTrigFactory(@asin, A, varargin{:});
            catch ME, throwAsCaller(ME); end
        end
        function new_obj = acos(A,varargin)
            try new_obj = Angle.inverseTrigFactory(@acos, A, varargin{:}); 
            catch ME, throwAsCaller(ME); end
        end
        function new_obj = atan(A,varargin)
            try new_obj = Angle.inverseTrigFactory(@atan, A, varargin{:}); 
            catch ME, throwAsCaller(ME); end
        end
        function new_obj = atan2(A,B,varargin)
            try new_obj = Angle.inverseTrigFactory(@atan2,A,B, varargin{:});
            catch ME, throwAsCaller(ME); end
        end
        
        function new_obj = acsc(A,varargin)
            try new_obj = Angle.inverseTrigFactory(@acsc, A, varargin{:}); 
            catch ME, throwAsCaller(ME); end
        end
        function new_obj = asec(A,varargin)
            try new_obj = Angle.inverseTrigFactory(@asec, A, varargin{:}); 
            catch ME, throwAsCaller(ME); end
        end
        function new_obj = acot(A,varargin)
            try new_obj = Angle.inverseTrigFactory(@acot, A, varargin{:}); 
            catch ME, throwAsCaller(ME); end
        end
    ...
    methods (Static, Access = private)        
        function new_obj = inverseTrigFactory(fcn, A, varargin)            
            ...
            if isequal(fcn, @atan2) % NOTE: (Rody Oldenhuis) make exception for atan2
                ...
                assert(isnumeric(A) || isa(A, 'Dimensionless') ...                           
                           || ( (isa(A, 'PhysicalQuantity') && isequal(A.intermediate_dimensions, Angle.dimensions)) ) ,...
                       [mfilename ':invalid_argument'], ...
